...
namespace Widgets
{
    ...
    bool Checkbox(const char* label, bool* v)
    {
        ...
        else
        {
            ...
            window->DrawList->AddRectFilled(check_bb.Min, check_bb.Max, ImColor(col_inactive));
        }
    ...
    int SubTab(const char* labels[], int count, int& current, ImVec2 start, float width)
    {
        ...
        for (int i = 0; i < count; i++)
        {
            ...
            if (i == current)
            {
                tcol = ImColor(Style::TabActiveTop);
                bcol = ImColor(Style::TabActiveBot);
            }
            else
            {
                tcol = ImColor(Style::TabInactiveTop);
                bcol = ImColor(Style::TabInactiveBot);
                if (hovered)
                {
                    tcol = ImColor(Style::TabHoveredTop);
                    bcol = ImColor(Style::TabHoveredBot);
                }
            }
            ...
            dl->AddLine(ImVec2(x0, top), ImVec2(x0, b.y), ImColor(Style::BorderInner));
            dl->AddLine(ImVec2(x1, top), ImVec2(x1, b.y), ImColor(Style::BorderInner));
            dl->AddLine(ImVec2(x0, top), ImVec2(x1, top), ImColor(Style::BorderInner));
            dl->AddLine(ImVec2(x0, b.y), ImVec2(x1 + 1, b.y), ImColor(Style::BorderInner));

            if (i == current)
                dl->AddRectFilled(ImVec2(x0, b.y - 1), ImVec2(x1, b.y), ImColor(Style::Accent));
    ...
    bool DockBarButton(ImU32 accent_color, const char* icon, bool& active)
    {
        ImGuiWindow* window = ImGui::GetCurrentWindow();
        if (window->SkipItems)
            return false;
        ...
        ImVec2 pos = window->DC.CursorPos;
        ImRect total_bb(pos, pos + ImVec2(button_size, button_size));
        ...
        ImU32 bg_color_top, bg_color_bottom;
        ImU32 border_color;
        ImU32 icon_color;

        if (active)
        {
            bg_color_top = ImColor(ImVec4(
                Style::Accent.x * 1.2f,
                Style::Accent.y * 1.2f,
                Style::Accent.z * 1.2f,
                Style::Accent.w
            ));
            bg_color_bottom = ImColor(Style::Accent);
            border_color = ImColor(Style::Accent);
            icon_color = IM_COL32(255, 255, 255, 255);
        }
        else
        {
            ImVec4 base_color = ImVec4(0x1A / 255.f, 0x1A / 255.f, 0x1F / 255.f, 1.0f);
            if (hovered)
            {
                base_color.x = ImClamp(base_color.x * 1.3f, 0.0f, 1.0f);
                base_color.y = ImClamp(base_color.y * 1.3f, 0.0f, 1.0f);
                base_color.z = ImClamp(base_color.z * 1.3f, 0.0f, 1.0f);
            }
            bg_color_top = ImColor(base_color);
            bg_color_bottom = ImColor(ImVec4(base_color.x * 0.8f, base_color.y * 0.8f, base_color.z * 0.8f, 1.0f));
            border_color = ImColor(Style::BorderInner);
            icon_color = hovered ? IM_COL32(220, 220, 220, 255) : IM_COL32(160, 160, 160, 255);
        }
        ...
        window->DrawList->AddRect(total_bb.Min, total_bb.Max, border_color, 0.0f, 0, 1.0f);

        if (active)
        {
            window->DrawList->AddRectFilled(
                total_bb.Min + ImVec2(1, 1),
                total_bb.Max - ImVec2(1, 1),
        ...
        ImVec2 icon_pos = ImVec2(
            total_bb.Min.x + (button_size - icon_size.x) * 0.5f,
            total_bb.Min.y + (button_size - icon_size.y) * 0.5f
        );
    ...
    void RenderTags()
    {
        ...
        for (auto& t : tags)
        {
            ...
            ImU32 col_top = ImColor(Style::InnerChildBg);
            ImU32 col_bottom = ImColor(Style::MainChildBg);

            float mid_y = rect_y + rect_h * 0.55f;
    ...
    bool Widgets::Dropdown(const char* id, const char* label, const std::vector<const char*>& items, int* cur, bool show_label)
    {
        ...
        if (open)
        {
            ...
            ImU32 ctop = ImColor(Style::FrameBg);
            ImU32 cbot = ImColor(Style::FrameBgHovered);
            ImU32 cbor = ImColor(Style::FrameBorder);
    ...
    bool DrawHotkey(Hotkey* hk, const char* lbl, bool show_lbl, ImGuiButtonFlags flags)
    {
        ...
        if (hk->KeyName()[0] != '\0') {
            ...
            if (ImGui::ItemAdd(cbb, cid)) {
                ...
                ImU32 ccol = cho ? IM_COL32(180, 130, 116, 255) : IM_COL32(160, 160, 160, 255);
    ...
    bool BeginFrameWindow(const char* title, const ImVec2& min_size)
    {
        ...
        Widgets::DrawBorder(dl, pos, size, ImColor(Style::Border));
        Widgets::DrawBorder(dl, pos, size, ImColor(Style::BorderInner), 2);

        ImGui::SetCursorPos(ImVec2(8, 8));
        ImGui::PushStyleColor(ImGuiCol_ChildBg, Style::MainChildBg);
        ImGui::BeginChild("Main", ImVec2(size.x - 16, size.y - 16), true);
        {
            ImVec2 cp = ImGui::GetWindowPos();
            ImVec2 cs = ImGui::GetWindowSize();
